...
def box_append_section_grid(  # pylint: disable=too-many-arguments
    ...
    *,
    ...
    frame = create_titled_frame(title, grid, title_class=title_class, frame_class=frame_class)
    box.append(frame)
    return grid


def box_append_blue_section_grid(box: Gtk.Box, title: str) -> Gtk.Grid:
    """Ajoute une sous-section "bleue" standard + une Grid.

    But: éviter la duplication (Pylint R0801) des mêmes paramètres
    (row/column spacing + classes CSS) dans plusieurs onglets.
    """
    return box_append_section_grid(
        box,
        title,
        row_spacing=12,
        column_spacing=12,
...
def create_titled_frame(
    title: str, child: Gtk.Widget, title_class: str | None = None, frame_class: str | None = None
) -> Gtk.Frame:
    """Crée un Gtk.Frame avec un titre intégré.

    Objectif: réduire la hauteur verticale en évitant le motif
    "titre (label) + widget" sur deux lignes.
    """
    frame = Gtk.Frame()
    frame.add_css_class("titled-frame")
    if frame_class:
    ...
    title_label.add_css_class("frame-title")
    if title_class:
        title_label.add_css_class(title_class)
...
def grid_add_labeled(
    ...
    *,
    ...
    if label_opts.css_class:
        label_widget.add_css_class(label_opts.css_class)
...
def grid_add_switch(
    ...
    switch: Gtk.Switch,
    *,
    label_opts: LabelOptions | None = None,
) -> int:
    ...
    opts = label_opts or LabelOptions()
    label_widget = Gtk.Label(label=label_text, xalign=opts.xalign)
    label_widget.set_hexpand(True)
    if opts.valign is not None:
        label_widget.set_valign(opts.valign)
    if opts.css_class:
        label_widget.add_css_class(opts.css_class)
...
def create_tab_grid_layout(root: Gtk.Box) -> Gtk.Grid:
    """Crée un Grid standard pour les onglets de configuration.

    Args:
        root: Container parent où ajouter le grid

    Returns:
        Grid configuré avec espacement standard
    """
    main_grid = Gtk.Grid()
...
def create_error_dialog(message: str, parent=None) -> None:
    ...
    if resolved_parent is None:
        try:
            ...
            if app is not None:
                resolved_parent = app.get_active_window()
    ...
    dialog.show(parent=resolved_parent)
