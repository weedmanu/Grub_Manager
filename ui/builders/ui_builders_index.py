...
from ui.tabs.ui_tabs_maintenance import build_maintenance_tab
...
class UIBuilder:
    ...
    def create_main_ui(window: GrubConfigManager) -> None:
        ...
        # UI plus compacte (moins de hauteur, moins de padding)
        ...
        window.notebook = UIBuilder.build_tabs(window, main_box)
    ...
    def _create_bottom_bar(window: GrubConfigManager, container: Gtk.Box) -> None:
        ...
        window.info_box.set_margin_top(6)
        window.info_box.set_margin_bottom(6)
        window.info_box.set_margin_start(8)
        window.info_box.set_margin_end(8)
        ...
        window.preview_btn = Gtk.Button(label="Aperçu")
        window.preview_btn.set_sensitive(False)  # Désactivé par défaut
        window.preview_btn.connect("clicked", window.on_preview)
        button_box.append(window.preview_btn)
    ...
    def _create_notebook(window: GrubConfigManager, container: Gtk.Box) -> Gtk.Notebook:
        ...
        notebook.append_page(theme_config.build_grub_scripts_tab(), Gtk.Label(label="Apparence"))
        notebook.append_page(theme_config.build_theme_tab(), Gtk.Label(label="Thèmes"))
