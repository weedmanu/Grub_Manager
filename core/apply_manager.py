...
from .grub_default import write_grub_default
...
class GrubApplyManager:
    ...
    def apply_configuration(self, new_config: dict[str, str], apply_changes: bool = True) -> ApplyResult:
        ...
        try:
            ...
            logger.error(f"[apply_configuration] Erreur durant l'étape {self._state.name}: {e}")
            error_msg = str(e)

            # Tentative de rollback si on a dépassé l'étape de backup
            if self._state in (ApplyState.WRITE_TEMP, ApplyState.GENERATE_TEST, ApplyState.VALIDATE, ApplyState.APPLY):
                logger.warning("[apply_configuration] Lancement du rollback...")
                try:
                    self._transition_to(ApplyState.ROLLBACK)
                    self._rollback()
                    return ApplyResult(
                        False, f"Échec: {error_msg}. Restauration effectuée.", ApplyState.ROLLBACK, details=error_msg
                    )
                    ...
                    return ApplyResult(
                        False,
                        f"Échec CRITIQUE: {error_msg}. Rollback échoué: {rollback_error}",
                        ApplyState.ERROR,
                        details=error_msg,
                    )

            self._transition_to(ApplyState.ERROR)
            return ApplyResult(False, f"Erreur: {error_msg}", ApplyState.ERROR, details=error_msg)
