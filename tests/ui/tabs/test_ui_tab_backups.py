...
import ui.tabs.ui_tabs_backups as ui_backups
...
def test_on_create_clicked_empty_tar(mock_controller):
    ...
    with (
        patch("ui.tabs.ui_tabs_backups.os.geteuid", return_value=0),
        patch("ui.tabs.ui_tabs_backups.create_grub_default_backup", return_value="/p"),
        patch("ui.tabs.ui_tabs_backups.os.path.exists", return_value=True),
        patch("ui.tabs.ui_tabs_backups.tarfile.open", return_value=mock_tar),
    ):
...
def test_refresh_list_size_formatting(mock_controller):
    ...
    for size in [500, 5000, 5000000]:
        with (
            patch("ui.tabs.ui_tabs_backups.list_grub_default_backups", return_value=["/p"]),
            patch("ui.tabs.ui_tabs_backups.os.path.getmtime", return_value=1234567890),
            patch("ui.tabs.ui_tabs_backups.os.path.getsize", return_value=size),
        ):
...
def test_on_create_clicked_no_root(mock_controller):
    with patch("ui.tabs.ui_tabs_backups.os.geteuid", return_value=1000):
        ui_backups._on_create_clicked(None, mock_controller, None)
...
def test_on_create_clicked_file_not_found(mock_controller):
    with (
        patch("ui.tabs.ui_tabs_backups.os.geteuid", return_value=0),
        patch("ui.tabs.ui_tabs_backups.create_grub_default_backup", return_value="/p"),
        patch("ui.tabs.ui_tabs_backups.os.path.exists", return_value=False),
    ):
        ui_backups._on_create_clicked(None, mock_controller, None)
...
def test_on_create_clicked_tar_error(mock_controller):
    with (
        ...
        patch("ui.tabs.ui_tabs_backups.create_grub_default_backup", return_value="/p"),
        patch("ui.tabs.ui_tabs_backups.os.path.exists", return_value=True),
        patch("ui.tabs.ui_tabs_backups.tarfile.open", side_effect=tarfile.TarError("Bad tar")),
    ):
        ui_backups._on_create_clicked(None, mock_controller, None)
...
def test_on_create_clicked_generic_exception(mock_controller):
    with (
        patch("ui.tabs.ui_tabs_backups.os.geteuid", return_value=0),
        patch("ui.tabs.ui_tabs_backups.create_grub_default_backup", side_effect=RuntimeError("Boom")),
    ):
        ui_backups._on_create_clicked(None, mock_controller, None)
...
def test_on_restore_clicked_no_dropdown(mock_controller):
    with (patch("ui.tabs.ui_tabs_backups.os.geteuid", return_value=0),):
        ui_backups._on_restore_clicked(None, mock_controller, None)
...
def test_on_restore_clicked_exception(mock_controller):
    ...
    with (
        patch("ui.tabs.ui_tabs_backups.os.geteuid", return_value=0),
        patch("ui.tabs.ui_tabs_backups.restore_grub_default_backup", side_effect=RuntimeError("Err")),
        patch("ui.tabs.ui_tabs_backups.confirm_action", side_effect=lambda cb, *_: cb()),
    ):
        mock_controller.backup_paths = ["/p"]
        ui_backups._on_restore_clicked(None, mock_controller, dropdown)
...
def test_on_restore_clicked_typed_exception_branch(mock_controller):
    ...
    with (
        patch("ui.tabs.ui_tabs_backups.os.geteuid", return_value=0),
        patch("ui.tabs.ui_tabs_backups.restore_grub_default_backup", side_effect=ValueError("Bad")),
        patch("ui.tabs.ui_tabs_backups.confirm_action", side_effect=lambda cb, *_: cb()),
    ):
        mock_controller.backup_paths = ["/p"]
        ui_backups._on_restore_clicked(None, mock_controller, dropdown)
...
def test_on_delete_clicked_typed_exception_branch(mock_controller):
    ...
    with (
        patch("ui.tabs.ui_tabs_backups.os.geteuid", return_value=0),
        patch("ui.tabs.ui_tabs_backups.delete_grub_default_backup", side_effect=ValueError("Bad")),
        patch("ui.tabs.ui_tabs_backups.confirm_action", side_effect=lambda cb, *_: cb()),
    ):
        mock_controller.backup_paths = ["/p"]
...
def test_on_delete_clicked_exception(mock_controller):
    ...
    with (
        patch("ui.tabs.ui_tabs_backups.os.geteuid", return_value=0),
        patch("ui.tabs.ui_tabs_backups.delete_grub_default_backup", side_effect=RuntimeError("Err")),
        patch("ui.tabs.ui_tabs_backups.confirm_action", side_effect=lambda cb, *_: cb()),
    ):
        mock_controller.backup_paths = ["/p"]
        ui_backups._on_delete_clicked(None, mock_controller, dropdown, None)
...
def test_on_restore_clicked_success(mock_controller):
    ...
    with (
        patch("ui.tabs.ui_tabs_backups.os.geteuid", return_value=0),
        patch("ui.tabs.ui_tabs_backups.restore_grub_default_backup"),
        patch("ui.tabs.ui_tabs_backups.confirm_action", side_effect=lambda cb, *_: cb()),
    ):
        mock_controller.backup_paths = ["/p"]
        ui_backups._on_restore_clicked(None, mock_controller, dropdown)
...
def test_on_delete_clicked_success(mock_controller):
    ...
    with (
        patch("ui.tabs.ui_tabs_backups.os.geteuid", return_value=0),
        patch("ui.tabs.ui_tabs_backups.delete_grub_default_backup"),
        patch("ui.tabs.ui_tabs_backups.confirm_action", side_effect=lambda cb, *_: cb()),
    ):
        mock_controller.backup_paths = ["/p"]
...
def test_refresh_list_os_error(mock_controller):
    dropdown = Gtk.DropDown.new_from_strings([])
    empty_label = Gtk.Label(label="")
    with patch("ui.tabs.ui_tabs_backups.list_grub_default_backups", side_effect=OSError("Err")):
        ui_backups._refresh_list(mock_controller, dropdown, empty_label)
...
def test_refresh_list_os_error_in_loop(mock_controller):
    ...
    with (
        patch("ui.tabs.ui_tabs_backups.list_grub_default_backups", return_value=["/p"]),
        patch("ui.tabs.ui_tabs_backups.os.path.getmtime", side_effect=OSError),
    ):
        ui_backups._refresh_list(mock_controller, dropdown, empty_label)
        model = dropdown.get_model()
        assert isinstance(model, Gtk.StringList)
...
def test_refresh_list_no_backups(mock_controller):
    ...
    with patch("ui.tabs.ui_tabs_backups.list_grub_default_backups", return_value=[]):
