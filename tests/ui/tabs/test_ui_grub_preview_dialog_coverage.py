...
def test_grub_preview_dialog_show(theme):
    ...
    with patch("ui.tabs.ui_grub_preview_dialog.Gtk.Window") as mock_window_class, \
...
def test_create_grub_preview_success(theme):
    ...
    entries = [
        MenuEntry(title="Ubuntu", id="u"),
        MenuEntry(title="Windows", id="w")
    ]

    with patch("ui.tabs.ui_grub_preview_dialog.GrubService.read_current_config", return_value=config), \
...
def test_create_grub_preview_exception(theme):
    ...
    with patch("ui.tabs.ui_grub_preview_dialog.GrubService.read_current_config", side_effect=OSError("Boom")), \
